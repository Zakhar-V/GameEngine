...
namespace Engine
{
	...
	public:
		...
		//! Move constructor.
		Array(Array&& _rhs) noexcept : m_size(_rhs.m_size), m_capacity(_rhs.m_capacity), m_data(_rhs.m_data)
		{
			_rhs.m_size = 0;
			_rhs.m_capacity = 0;
			_rhs.m_data = nullptr;
		...
		//!	Move assignment.
		Array& operator = (Array&& _rhs) noexcept
		{
		...
		Iterator Erase(ConstIterator _pos) { return Erase((uint)(_pos - m_data), 1); }
		//!	Erase a range of elements by iterators. \return iterator to the next element. 
		Iterator Erase(ConstIterator _start, ConstIterator _end) { return Erase((uint)(_start - m_data), (uint)(_end - m_data)); }
		...
		Iterator Insert(ConstIterator _pos, const T& _src) { return Insert((uint)(_pos - m_data), &_src, 1); }
		//! Insert an elements in position.
		Iterator Insert(ConstIterator _pos, const T* _src, uint _count) { return Insert((uint)(_pos - m_data), _src, _count); }
		//! Insert an elements in position.
		Iterator Insert(uint _pos, const T* _src, uint _count)
		{
			...
			else
			{
				...
				for (T* _end = (_iEnd < _cEnd ? _cEnd : _iEnd); _dst > _end;)
		...
		template<class... Args> Iterator Emplace(uint _pos, Args&&... _args)
		{
			...
			else
			{
				...
				uint _newSize = m_size + 1;
				for (T* dst = m_data + _newSize, *src = m_data + _pos, *end = m_data + m_size; src < end;)
	...
	public:
		...
		Iterator Insert(ConstIterator _iter)
		{
			...
			uint _hash = _insert->m_hash;
			Node* _node = _Find(_insert->m_value.first, _hash);
			if (!_node)
			{
				_node = _Insert(_hash);
				Construct(const_cast<T*>(&_node->m_value.first), _insert->m_value.first);
				Construct(&_node->m_value.second, _insert->m_value.second);
			}
			else
		...
		//!
		bool Contains(const T& _key) const
		{
